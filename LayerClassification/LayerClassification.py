...
# Parameters
DATA_DIR = r"c:\Aziz\MasterThesis\MasterTh\Super\SO_SP_SR_SLMwithShufle"  # Root folder containing train, val, test folders
...
fc = 0.8                # Central frequency of the Morlet wavelet (approximately)
...
# Function to load data from a specified folder (train, val, test)
def load_data_from_folder(dataset_dir, scales_param):
    X_list = []
    y_list = []
    for label, class_folder in enumerate(["SO", "SP", "SR", "SLM"]):
        ...
        # Discard files where processing failed (None)
        feats = [f for f in feats if f is not None]
        if feats:
            X_file = np.vstack(feats)
            X_list.append(X_file)
            # The number of windows for each file equals f.shape[0]
...
class_names = ["SO", "SP", "SR", "SLM"]  # four classes

conf_matrix = confusion_matrix(y_test, y_test_pred)

plt.figure(figsize=(6,5))
sns.heatmap(conf_matrix, annot=True, fmt="d", cmap="Blues", cbar=False,
            xticklabels=[f"Pred {cn}" for cn in class_names],
            yticklabels=[f"Actual {cn}" for cn in class_names])
plt.ylabel("Actual")
plt.xlabel("Predicted")
plt.title("Confusion Matrix (4-Class: SO, SP, SR, SLM)")
plt.show()